...
function getLatestUserReportMessage (reporterId) {
    return connection.queryAsync('SELECT * FROM message INNER JOIN report ON message.reportId = report.id AND message.reportId = (SELECT id FROM report WHERE reporterId = ? ORDER BY id DESC LIMIT 1)',
        [reporterId]);
}
